...
def draw_corners(image, bbox, color=(0, 255, 0), thickness=3, proportion=0.2):
    """
    Draw corner markers on bounding box (same as draw_bbox but only corners).
    
    Args:
        image: Image to draw on
        bbox: Bounding box [x1, y1, x2, y2, score] or [x1, y1, x2, y2]
        color: Color tuple (B, G, R)
        thickness: Line thickness
        proportion: Proportion of box size for corner length
    """
    # Handle bbox with or without score
    if len(bbox) >= 5:
...
def draw_keypoints(image, keypoints, color=(0, 255, 255), radius=3, thickness=-1):
    """
    Draw facial keypoints (landmarks) on image.
    
    Args:
        image: Image to draw on
        keypoints: Array of shape (5, 2) or (n_faces, 5, 2) - 5 facial landmarks
                  [left_eye, right_eye, nose, left_mouth, right_mouth]
        color: Color tuple (B, G, R) for keypoints
    ...
    if len(keypoints.shape) == 2:
        ...
        # Multiple faces: (n_faces, 5, 2)
        for face_kps in keypoints:
            for i, (x, y) in enumerate(face_kps):
                x, y = int(x), int(y)
                cv2.circle(image, (x, y), radius, color, thickness)
    else:
        # Flatten and try to reshape
        kps = np.array(keypoints).reshape(-1, 2)
        for i, (x, y) in enumerate(kps[:5]):  # Take first 5 keypoints
            x, y = int(x), int(y)
            cv2.circle(image, (x, y), radius, color, thickness)
    
    return image

def extract_images_from_payload(payload: dict) -> list[str]:
    """
    Normalize webhook payload and extract ALL images as base64 strings.
    Supports:
      - image
      - result_image
      - image_data
      - images: []
      - frames: [{ image }]
      - cropped_detections: [{ cropped_image }]
    """
    images = []

    # Single-image fields
    for key in ("image", "result_image", "image_data"):
        if payload.get(key):
    ...
    # Cropped detections (VERY IMPORTANT for your case)
    if isinstance(payload.get("cropped_detections"), list):
        for det in payload["cropped_detections"]:
            if det.get("cropped_image"):
                images.append(det["cropped_image"])
