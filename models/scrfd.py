...
if __name__ == "__main__":
    ...
    while True:
        ...
        # detect() returns: (det, kpss)
        # det: shape (n_faces, 5) - [x1, y1, x2, y2, confidence]
        # kpss: shape (n_faces, 5, 2) - 5 landmarks per face
        det, kpss = detector.detect(frame, max_num=10)

        if det is not None and len(det) > 0:
            for i in range(len(det)):
                x1, y1, x2, y2, score = det[i].astype(np.int32)
                # Draw bounding box corners
                draw_corners(frame, [x1, y1, x2, y2], color=(0, 255, 0), thickness=2)
                
                # Draw keypoints if available
                if kpss is not None and len(kpss) > i:
                    draw_keypoints(frame, kpss[i], color=(0, 255, 255), radius=3)
