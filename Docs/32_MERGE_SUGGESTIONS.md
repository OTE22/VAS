...
> [`23_VECTOR_INDEX_CONTRACT.md`](23_VECTOR_INDEX_CONTRACT.md). The
...
**See also:** [Graph-Based Clustering Guide](./32_MERGE_SUGGESTIONS.md) for detailed explanation of the graph approach.
...
## How the candidates are found: graph clustering, pipeline awareness, ML scoring

*(Absorbed 2026-09-12 from the former 11_GRAPH_BASED_CLUSTERING, 41_ML_CLUSTERING and
41_PIPELINE_AWARE_ML_CLUSTERING_GUIDE. Verified against
`backend/core/identity_clustering.py`, `backend/core/pipeline_aware_clustering.py` and
the `/api/admin/merge-suggestions*` routes.)*

### Graph clustering, not pairs

Comparing identities two at a time finds pairs and misses groups: if A≈B and B≈C but
A and C are just below the threshold, the pair approach produces two suggestions and
leaves the operator to discover that all three are one person. The clustering step
therefore builds a **similarity graph** — identities are nodes, an edge exists where
similarity clears the threshold — and takes its **connected components** as clusters.
One suggestion then covers the whole group, which is also why the merge UI offers
multi-identity merge with a preview (`33_MULTI_IDENTITY_MERGE.md`).

### Pipeline-aware clustering

`POST /api/admin/merge-suggestions/generate-pipeline-aware` runs the same idea per
camera and respects who may see what:

1. **Filter by the caller's cameras** — suggestions never span pipelines the operator
   cannot access (the endpoint is admin-only, and the per-pipeline variant
   `GET /api/admin/merge-suggestions/pipeline/{pipeline_id}` clusters one camera's
   unknowns with DBSCAN).
2. **Features** beyond raw cosine similarity: how often the identities appear, on which
   cameras, at which times, and the quality of the crops behind each embedding.
3. **Quality weighting** — a match built on two blurred crops is not worth the same as
   one built on two sharp frontal faces.

### Where the ML model fits

The heuristic score (similarity plus pattern agreement) is the default. When a trained
similarity model is **active** in the registry it scores the same candidate pairs
instead, learning from the merges operators accepted and rejected — training, promotion
and rollback are operator actions on `/admin/ml-model`, described in
`50_ML_SIMILARITY_MODEL.md`. Nothing is merged automatically in either mode: a
suggestion is a proposal, and `POST /api/admin/merge-suggestions/{id}/approve` is the
only thing that changes data.
