...
This is documented in `utils/logging.py:59-88` with measurements. The
...
|---|---|---|
| Container has no GPU access | `$DC exec face_recognition nvidia-smi` fails | you are on the CPU stack, or the NVIDIA container toolkit is missing — see `03_GPU_SETUP.md`. Production GPU **layers** `prod.gpu.yml` on `prod.yml`; using `gpu.yml` alone silently drops backups and monitoring. |
...
Before anything destructive: [`11_BACKUP_AND_RESTORE.md`](11_BACKUP_AND_RESTORE.md).
...
**See also:** [`14_ADMIN_CHEAT_SHEET.md`](14_ADMIN_CHEAT_SHEET.md) ·
[`04_DEPLOYMENT_RUNBOOK.md`](04_DEPLOYMENT_RUNBOOK.md) ·
