...
- **[32_MERGE_SUGGESTIONS.md](./32_MERGE_SUGGESTIONS.md)** - Automatic merge suggestions
- **[32_MERGE_SUGGESTIONS.md](./32_MERGE_SUGGESTIONS.md)** - Graph-based clustering algorithm
- **[34_PROMOTION_FLOW.md](./34_PROMOTION_FLOW.md)** - General merge guide
- **[30_UNKNOWN_FACES_CENTER.md](./30_UNKNOWN_FACES_CENTER.md)** - Unknown faces management
...
## What a merge preserves, and the two safety gates

*(Absorbed 2026-09-12 from the former 37_ADVANCED_MERGE_FLOW_GUIDE. Verified against
`backend/routes/identities.py`: `preview_merge`, `merge_identities`,
`merge_multiple_identities`, `unmerge_identities`.)*

### Pipeline preservation

Merging identities from different cameras does **not** discard the camera history. Every
appearance row keeps its own `pipeline_id`, and the surviving identity therefore inherits
the union of the cameras. That is what makes cross-camera tracking possible after a merge:
the person now has one record that was seen at the gate and at the car park, with the
times and the cameras intact. The preview reports this as the **pipeline distribution**,
so an operator can see before committing that they are about to join a gate identity to a
car-park identity.

### The preview is a read

`POST /api/admin/identities/merge-preview` `{identity_ids, target_identity_id?}` changes
nothing. It answers four questions: which identity would survive (target choice), what the
combined camera distribution would be, whether the **type** would be promoted (merging a
known person with unknowns produces a known person), which snapshot would represent the
result, and the row counts involved.

### The risk gate

`merge-multiple` and `approve` take `confirm_merge_risk`. The UI asks for that confirmation
when the preview flags a risky combination — typically identities that never co-occur on a
camera, or a type conflict. Sending `confirm_merge_risk: true` without showing a human the
preview defeats the gate; it exists because a wrong merge blends two people into one record.

### A merge is reversible

The loser keeps `merged_into_id`, the event is recorded in `identity_merges`, and
`POST /api/admin/identities/merges/{merge_id}/unmerge` reverses one merge. Vector rows and
appearances follow the same path back. Reversibility is why the system never merges
automatically — a suggestion or a clustering result is a proposal, not an action.
