...
- **05_ADMIN_SETUP.md** - How to set up users and grant pipeline access
- **34_PROMOTION_FLOW.md** - Detailed guide on promoting and merging
- **30_UNKNOWN_FACES_CENTER.md** - Complete unknown faces guide
- **45_API_AUTHENTICATION.md** - API authentication and token usage
...
## Granting access to Unknown Faces

*(Absorbed 2026-09-12 from the former 27_HOW_TO_GRANT_UNKNOWN_FACES_ACCESS.)*

A non-admin reaches **Unknown Faces** only when an administrator has assigned at least one
camera to the account: the page is scoped to the caller's pipelines, and with none assigned
it would show nothing. Assign cameras on **Admin → Users** (the pipeline checkboxes in the
user modal — `46_WEB_PAGES_REFERENCE.md` §6), which writes `user_pipeline_access` and bumps
`permissions_version` so the user's open sessions re-evaluate immediately. The navbar link
appears on the user's next page load because the backend emits it in
`GET /api/auth/me/privileges`; the frontend never decides visibility by itself.
