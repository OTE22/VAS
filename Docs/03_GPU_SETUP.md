...
## GPU detection in the application (absorbed from the former 13_README_GPU)

The application detects the GPU at startup rather than being told about it: when CUDA and a
usable device are present the inference session runs on the GPU, otherwise it falls back to
the CPU and says so in the log. That makes the same image valid on both kinds of host — the
compose overlay decides whether the device is offered to the container at all
(`docker/docker-compose.gpu.yml` for development, `docker/docker-compose.prod.gpu.yml`
layered over the production file).

Verify what actually happened, rather than assuming:

```bash
docker compose -f docker/docker-compose.gpu.yml exec face_recognition nvidia-smi   # device visible in the container?
docker compose -f docker/docker-compose.gpu.yml logs face_recognition | grep -i -E "cuda|gpu|provider"
```

Expect the log to name the execution provider it chose. If it says CPU on a GPU host, the
container was started without the GPU overlay, or the NVIDIA Container Toolkit is not
configured for the Docker daemon — the steps above in this document fix exactly that.
