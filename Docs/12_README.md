...
> **Scope note.** This is the executive / architecture overview. For anything
> operational use the root [`README.md`](../README.md),
> [`61_DEPLOYMENT_RUNBOOK.md`](61_DEPLOYMENT_RUNBOOK.md) and
> [`75_API_REFERENCE.md`](75_API_REFERENCE.md), which are kept current against
> the code. Stack details below were corrected in 2026-08: the database is
> **PostgreSQL 15 + pgvector**, never SQLite, and recognition uses
...
Version: see `VERSION` in config.py | Build: Production | Classification: RESTRICTED
...
```
 ...
 • Camera Feed N       │         │ • Chatbot Interface                │
  ...
  └───────────────────────────────┬──────────────────────────────────┘  │
         ...
         ↓                          ↓                          ↓
      ┌─────────────────┐      ┌──────────────────┐
  PostgreSQL     │      │  Ollama AI      │      │  3-Hour Cache    │
      ├─────────────────┤      ├──────────────────┤
...
- **Dockerfiles:** [`docker/Dockerfile.cpu`](../docker/Dockerfile.cpu) and
  [`docker/Dockerfile.gpu`](../docker/Dockerfile.gpu). The shape below is the
  dependency layer as it actually is — read the real files before changing
  anything, they carry the reasoning in comments.
  ...
  # requirements-base.txt MUST be copied alongside the hardware file: the
  # hardware file starts with `-r requirements-base.txt`, and pip resolves that
  # include relative to the file, so the build fails immediately without it.
  ...
  There is **no `requirements.txt`** anywhere in this repository. Dependencies
  are split three ways — `requirements-base.txt` (shared),
  `requirements-cpu.txt` and `requirements-gpu.txt` (only what genuinely
  differs by hardware) — plus `requirements-dev.txt`, installed only when the
  image is built with `--build-arg INSTALL_DEV=true`. The entrypoint is
  `gunicorn backend.main:app`, not `uvicorn main:app`.
...
Per-feature documentation lives in the numbered guides rather than here —
see [`00_DOCUMENTATION_INDEX.md`](00_DOCUMENTATION_INDEX.md). This section
was an unwritten placeholder and has been replaced with that pointer.
...
- Python (FastAPI, OpenCV, ONNX Runtime, PostgreSQL + pgvector)
...
- Database (PostgreSQL, pgvector, SQL optimization)
