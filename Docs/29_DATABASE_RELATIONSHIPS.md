# Database Relationships — the 63 tables as one connected model (verified 2026-09-12, head `fcc3d4e5f6a7`)

How every application table connects to every other one, proven from three
sources cross-checked against each other: the ORM (`db_models.py` and the model modules, 63
`__tablename__` classes), the Alembic migrations (49 revisions, root `000_baseline_schema`), and the
