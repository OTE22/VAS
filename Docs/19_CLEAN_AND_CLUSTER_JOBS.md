...
## Cleaning up unknown identities (absorbed from the former 23)

Unknown identities accumulate by design: every face that does not match becomes one. Three
mechanisms keep that bounded, and they are different things:

- **Retention** removes data older than the configured window — detections, faces, snapshots
  and the embeddings that belong to them. It is scheduled, and can be run on demand (with a
  dry run first) from **Admin → Settings** or Background Tasks.
- **Clustering** does not delete anything; it proposes merges so that fifty sightings of one
  stranger become one identity (`32_MERGE_SUGGESTIONS.md`).
- **A deliberate wipe** is an operator action with its own procedure and safety rails —
  `15_CLEARING_DATA.md`. Use the maintained scripts; ad-hoc `DELETE` statements leave
  the vector rows, the files on disk and the audit trail inconsistent with each other.

Order matters when you want a clean slate: merge or promote what you want to keep **first**,
then let retention or the wipe take the rest, because a merge re-parents rows that a delete
