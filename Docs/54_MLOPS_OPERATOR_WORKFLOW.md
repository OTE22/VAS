...
## Evidence, limits and how to verify without user data

*(Absorbed 2026-09-12 from the former 54_MLOPS_OPERATOR_WORKFLOW.md.)*

**What the guided workflow is.** The ML-Ops page walks an operator through
check → prepare → review → decide in a fixed order, because each stage's output is the next
stage's input: features feed datasets, datasets feed training, training produces a candidate,
and only shadow evidence justifies a promotion decision. The page's "next step" button is a
convenience over that order, not a separate pipeline.

**The limits, stated plainly.** Rules remain the decision system; a model runs in shadow and
changes nothing until an administrator promotes it. A candidate with insufficient or
conflicting labels is refused with a named reason rather than trained on anyway
(`51_BEHAVIORAL_ANOMALY_ML.md` lists the refusals). Promotion, rejection and rollback
are audited.

**Verifying without touching user data.** The workflow can be exercised end to end on
synthetic or historical data: build a dataset over a past date range, train, and read the
shadow summary and evidence — no live traffic is required, and nothing the shadow model
predicts reaches an operator's screen as a decision.
