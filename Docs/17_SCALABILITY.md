...
# PGVECTOR_HNSW_M / _EF_CONSTRUCTION / _EF_SEARCH — see 23_VECTOR_INDEX_CONTRACT.md.
...
- **Performance Optimization Guide:** `Docs/17_SCALABILITY.md`
- **FAISS Scaling Guide:** `Docs/23_VECTOR_INDEX_CONTRACT.md`
...
## Throughput arithmetic (absorbed from the former 17_CAPACITY_VERIFICATION)

The question "can it take N cameras" reduces to one comparison: **frames offered per second
vs frames processed per second**. Frames offered is `cameras × frames-per-second-per-camera`
(cameras send on motion, so the sustained figure is far below the peak). Frames processed is
`QUEUE_WORKERS × (1 / seconds-per-frame)`, where seconds-per-frame is detection plus
embedding plus the database write, and is dominated by the GPU when one is present.

Read the two numbers from the running system rather than from this page: the queue depth and
processed counters on **Home** (`/api/stats`), and the per-stage timings in the logs. If the
queue depth grows over a sustained period, the offered rate exceeds the processed rate and no
amount of tuning elsewhere changes that — add workers, add GPU, or reduce the offered rate.
The figures quoted in this document were measured on the hardware of the day and should be
re-measured after any hardware or model change.
