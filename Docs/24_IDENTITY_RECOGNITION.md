...
## The two models behind every embedding

*(Absorbed 2026-09-12 from the former 34_SCRFD_ARCFACE_INTEGRATION_PIPELINE. Verified
against `models/` and the detection/recognition services.)*

| Stage | Model | Input | Output |
|---|---|---|---|
| **Detect** | SCRFD (`scrfd.py`) | the frame, resized to 640×640 | face boxes + five landmarks per face |
| **Align** | — | the landmarks | the face warped to a canonical 112×112 crop |
| **Embed** | ArcFace (`arcface.py`) | the 112×112 crop | a 512-dimensional unit vector |

Two consequences worth remembering:

- **Alignment is why landmarks matter.** A box alone gives a crop that varies with pose;
  warping onto the five landmarks is what makes two pictures of the same person land close
  together in the embedding space. A detection without usable landmarks yields a weaker
  embedding, which is part of what the quality score reflects
  (`28_IMAGE_QUALITY_ANALYSIS.md`).
- **Similarity is cosine distance between unit vectors**, so a threshold is a number
  between 0 and 1 and is directly comparable across cameras. The thresholds themselves are
  settings (`06_CONFIGURATION_GUIDE.md`), and the vector store that holds these embeddings
  is described in `23_VECTOR_INDEX_CONTRACT.md`.

Model files live under `models/` and are loaded once per worker process; both run on the
GPU when one is present and on the CPU otherwise (`03_GPU_SETUP.md`).
