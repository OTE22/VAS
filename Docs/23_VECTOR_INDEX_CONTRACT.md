...
**Status:** live, and implemented. `backend/core/vector_index/` **is** this
contract in code — `base.py` states the same two rules in its module docstring,
`factory.py` selects the backend and fails closed rather than downgrading
silently, `pgvector_index.py` is the production implementation, `flat_faiss.py`
the alternative, and `reconcile.py` proves the index can be rebuilt from the
database. It is imported by `backend/lifespan.py`, `backend/core/model_manager.py`,
`backend/core/identity_clustering.py` and `backend/routes/enrollment_review.py`.
(An earlier revision of this document said the package was deleted; that was
wrong — what was deleted is the *old* `IdentityIndexService` design described
further down.)

For how an image reaches this index in the first place — enrollment, camera
ingest, quality scoring and recognition — see
**`Docs/20_IMAGE_INGESTION_WORKFLOW.md`**.

Anything in the older FAISS-era documents that describes `IdentityIndexService`,
...
*(Absorbed 2026-09-12 from the former 35_PGVECTOR_INTEGRATION.)*

| Setting | Default | Meaning |
|---|---|---|
| `VECTOR_BACKEND` | `pgvector` | `pgvector` (recommended for production) or `faiss` |
| `PGVECTOR_INDEX_TYPE` | `hnsw` | `hnsw` (fast, recommended) or `ivfflat` (lower memory) |

The extension is created by the database image and the initialisation SQL; embeddings are a
`vector` column on the embedding table, and similarity search is an ORDER BY on the cosine
distance operator, which the index accelerates. Because the column **is** the data, a
rebuilt or dropped index costs query time and nothing else — no re-enrollment, no
reconciliation with a separate file, which is the whole point of the contract above.
