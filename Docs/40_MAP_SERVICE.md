...
build procedure live in [`41_MAP_DATASET_ACQUISITION.md`](41_MAP_DATASET_ACQUISITION.md);
the migration record and the defect post-mortem live in
[`40_MAP_SERVICE.md`](40_MAP_SERVICE.md) and
[`40_MAP_SERVICE.md`](40_MAP_SERVICE.md).
...
[`13_TROUBLESHOOTING.md`](13_TROUBLESHOOTING.md) §9.

---

## Five rules the offline map stack enforces

*(Absorbed 2026-09-12 from the former 89_OFFLINE_MAP_REMEDIATION, which recorded a defect
where a basemap archive contained 145,718 copies of an "access blocked" image and passed
every file-level check. These are the checks that replaced those.)*

1. **Content is measured, and unmeasured content is unusable.** Presence, size and checksum
   prove nothing about what is inside a tile archive. Tiles are sampled and characterised
   (distinct-image ratio, degenerate-content detection), and the measurement — not the file
   — decides whether a dataset may serve.
2. **A ledger records the verdict, and availability fails closed.** Each dataset has a
   verdict (`ACTIVE`, `PENDING`, refused-with-a-reason). Anything not positively verified is
   treated as unavailable, so the map degrades visibly instead of silently serving rubbish.
3. **Every refusal has a name.** `CONTENT_MISSING`, `CONTENT_NOT_VERIFIED`,
   `CONTENT_DEGENERATE`, `CHECKSUM_MISMATCH`, `ARCHIVE_CORRUPT`, `BUILD_INCOMPLETE`,
   `MARTIN_UNREACHABLE` — the API says which one, so an operator is never left with "the map
   does not work".
4. **Replacing a dataset is crash-safe.** A new archive is written beside the old one and
   swapped atomically, with the previous verdict revoked under the same key; an interrupted
   install leaves the old dataset serving, never a half-written one.
5. **Builds are independent and hardened.** Each dataset is built and verified on its own on
   the preparation machine (`41_MAP_DATASET_ACQUISITION.md`); a failure in one does not
   produce a partially-good bundle for the air-gapped server.
