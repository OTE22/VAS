...
## With pgvector as the vector backend

*(Absorbed 2026-09-12 from the former 42_ML_PGVECTOR_INTEGRATION.)*

With `VECTOR_BACKEND=pgvector` (the production default) the model's features come from the
database rather than from a separate index process: candidate pairs and their similarities
are computed in PostgreSQL, and the model scores the pairs the query returns. That removes
the need to keep a second structure in step with the database, which is the failure mode the
vector-index contract exists to prevent (`23_VECTOR_INDEX_CONTRACT.md`) — PostgreSQL is
authoritative, and the index is a disposable acceleration layer with `PGVECTOR_INDEX_TYPE`
(`hnsw` by default, `ivfflat` where memory is tight) as its only real knob.

Model artifacts are tracked in the **model registry**, and the registry — not the
filesystem — is what activation, rollback and the readiness gates consult: list versions
with `GET /api/admin/merge-suggestions/models` and read `model-status` for the sample
counts actually used. The on-disk artifact path still exists as the setting
`SIMILARITY_MODEL_PATH` (default `models/similarity_model.pkl`); treat it as where an
artifact lands, not as the source of truth for which model is active.
