...
**Note**: These features are documented in detail in **Chapter 9** (`38_SECURITY_INTELLIGENCE.md`).
...
## Cross-camera analysis: co-appearance, correlation, trajectory, thresholds

*(Absorbed 2026-09-12 from the former 57, 58, 59, 62, 67 and 83, which documented
the same four capabilities across six documents and taught the deprecated
synchronous threshold endpoint. Everything below is verified against
`config.py`, `backend/routes/intelligence.py` and `backend/core/security_intelligence_service.py`.)*

### What the four capabilities do

| Capability | What it establishes | Where it surfaces |
|---|---|---|
| **Cross-camera co-appearance** | Two people are related when they are seen by **different** cameras close in time and space — the basis of the social graph. Same-camera co-appearance alone misses people moving through a site | `GET /api/security/network`, `GET /api/identities/{id}/related` |
| **Activity correlation (xCCA)** | Temporal association: do two identities' activity patterns rise and fall together, even when never seen in the same frame | `GET /api/intelligence/correlation/calculate?identity_a&identity_b` |
| **Trajectory prediction** | Which camera an identity is likely to reach next, from the movement history between cameras | `GET /api/intelligence/trajectory/predict?identity_id&current_camera` |
| **Learned thresholds** | Per-camera-pair time/distance thresholds learned from observed transitions instead of one global guess | `POST /api/intelligence/thresholds/jobs` (background job) → `GET /api/intelligence/thresholds/jobs/{job_id}`; activate with `POST /api/security/learned-thresholds/{id}/activate` |

**Use the job endpoint, not the old synchronous one.** `POST /api/intelligence/thresholds/learn`
still exists but is documented in the code as the *deprecated* variant; it holds a
request open while it learns. The job form is single-flight, survives a restart and
reports progress in Background Tasks.

### Settings that govern them (defaults from `config.py`)

| Setting | Default | Meaning |
|---|---|---|
| `MULTI_CAMERA_CO_APPEARANCE_ENABLED` | `true` | cross-camera co-appearance detection on/off |
| `MULTI_CAMERA_TIME_WINDOW_MINUTES` | `10` | how far apart in time two sightings may be |
| `MULTI_CAMERA_DISTANCE_METERS` | `500.0` | how far apart the two cameras may be (needs camera coordinates — set them on `/admin/pipelines`) |
| `MULTI_CAMERA_MIN_CO_APPEARANCES` | `2` | co-appearances required before a relationship is asserted |
| `ACTIVITY_CORRELATION_ENABLED` | `true` | xCCA analysis on/off |
| `TRAJECTORY_PREDICTION_ENABLED` | `true` | next-camera prediction on/off |
| `AUTO_THRESHOLD_LEARNING_ENABLED` | `true` | allow learned per-pair thresholds |

Camera coordinates are a prerequisite for the distance rule and for every map view:
without them a camera pair has no distance and cross-camera co-appearance cannot be
established. Relationship caches are rebuilt by
`POST /api/intelligence/relationships/calculate-all` (a background job) or per identity
by `POST /api/identities/{id}/related/refresh`.

### Reading a result honestly

These are **association** measures, not proof of association: a shared workplace, a
shared bus route and a shared intent look alike to a camera network. The system reports
the evidence (counts, windows, cameras) alongside every score for that reason, and the
risk engine keeps scores separate from decisions — see `39_RISK_PLATFORM.md`.
