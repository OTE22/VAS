...
## Findings worth keeping from the September 2026 verification

*(Absorbed 2026-09-12 from the former 95_BACKGROUND_JOBS_VERIFICATION report.)*

- **`GET /api/tasks/upcoming` is empty by design** when the schedulers compute their next
  run lazily; an empty list is not a sign that jobs are not scheduled. Judge scheduling by
  `GET /api/tasks/history` and the running list instead.
- **Overlap protection is three-layered** (in-process guard, database claim, and the
  distributed lock for multi-worker deployments), so a job cannot run twice concurrently
  even with several API workers — which is why a "still running" status can persist
  briefly after a container restart until the lease expires.
- **Log noise that is not a job problem:** a scheduler logging "no work to do" every cycle,
  and retention reporting zero deletions on a young deployment.
- Administrators control the jobs through `POST /api/tasks/{id}/cancel` and `/retry` and
  through the retention run on the Settings page; the schedules themselves come from
  settings, not from code edits.
