...
def next_image_filename(folder: str, extension: str) -> str:
    """image_001.jpg, image_002.jpg, ... — generated by us, not by the client."""
...
def prepare_upload(
    ...
    *,
    ...
    _bbox, landmarks = detect_face_landmarks(image, is_face_image)
...
def consolidate_image_file(stored_relative_path: str, winner_identity_id) -> Tuple[Optional[str], bool]:
    ...
    function can lose a file, and `next_image_filename` scans the destination
    folder, so it can never overwrite one either (a collision simply yields the
    next free image_NNN slot).
    ...
    try:
        ...
        destination = os.path.join(folder, next_image_filename(folder, extension))
...
async def enroll_image(
    ...
    *,
    ...
    try:
        ...
        os.makedirs(folder, exist_ok=True)
        filename = next_image_filename(folder, extension)
