...
class IntelligenceService:
    ...
    async def _calculate_co_appearances(
        ...
        # Get all appearances of target identity
        ...
        multi_camera_enabled = getattr(settings, 'MULTI_CAMERA_CO_APPEARANCE_ENABLED', True)
        
        # Use learned thresholds if available, otherwise use defaults
        from backend.core.threshold_learner import threshold_learner
        
        # Get default thresholds
        multi_camera_distance = getattr(settings, 'MULTI_CAMERA_DISTANCE_METERS', 500.0)
        multi_camera_time_window = getattr(settings, 'MULTI_CAMERA_TIME_WINDOW_MINUTES', 10)
        multi_camera_min = getattr(settings, 'MULTI_CAMERA_MIN_CO_APPEARANCES', 2)
        
        # Check if automatic threshold learning is enabled
        use_learned_thresholds = getattr(settings, 'AUTO_THRESHOLD_LEARNING_ENABLED', True)
        ...
        for pipeline_id, window_start, window_end, app_time in time_windows:
            # Query for other identities appearing in this time window on the same pipeline
            query = select(IdentityAppearance).where(
                and_(
                    ...
                    or_(
                        IdentityAppearance.end_time >= window_start,
                        IdentityAppearance.end_time == None
                    )
                )
            )
            result = await db.execute(query)
        ...
        if multi_camera_enabled and pipeline_coords:
            ...
            if not pipelines_with_coords:
                ...
                for target_pipeline_id, window_start, window_end, app_time in time_windows:
                    nearby_pipelines = nearby_pipelines_cache.get(target_pipeline_id, [])
                    
                    if not nearby_pipelines:
                        continue
                    
                    # Use larger time window for cross-camera (people may travel between cameras)
                    cross_camera_window_start = app_time - timedelta(minutes=multi_camera_time_window)
                    cross_camera_window_end = app_time + timedelta(minutes=multi_camera_time_window)
                    ...
                    try:
                        query = select(IdentityAppearance).where(
                            and_(
                                ...
                                or_(
                                    IdentityAppearance.end_time >= cross_camera_window_start,
                                    IdentityAppearance.end_time == None
                                )
                            )
                        ).limit(1000)  # Safety limit to prevent excessive queries
        ...
        for other_id, data in filtered.items():
            ...
            co_appearance_pct = (data['count'] / total_target_appearances) * 100 if total_target_appearances > 0 else 0
            ...
            if getattr(settings, 'ACTIVITY_CORRELATION_ENABLED', True):
                try:
                    from backend.core.activity_correlation import activity_correlation_analyzer
                    correlation_score, _ = await activity_correlation_analyzer.calculate_correlation(
