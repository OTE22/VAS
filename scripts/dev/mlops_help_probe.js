/**
 * ML-Ops help/tooltips/progress/call-log probe (mlops-4).
 *   node scripts/dev/mlops_help_probe.js [--base http://localhost]
 * Opens every section's help modal (click + keyboard close), checks tooltips
 * on controls, the call-log card (with request ids), and the job stage
 * strip markup. Read-only except one harmless GET sequence.
 */
...
(async () => {
  ...
  try {
    ...
    check("every card has a ? help button", helpCount >= 13, `${helpCount} buttons`);
    ...
    const calls = await page.evaluate(() => {
      const rows = [...document.querySelectorAll("#calls-body .mlops-call-row")];
    ...
    const errRows = await page.evaluate(() => [...document.querySelectorAll("#calls-body .mlops-call-row")].map(r => r.classList.contains("call-error")));
